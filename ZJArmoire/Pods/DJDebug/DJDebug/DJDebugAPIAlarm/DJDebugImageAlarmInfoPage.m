...
#import "DJDebugImageAlarmInfoPage.h"
#import "IASKSpecifier.h"
#import "DJDebugURLProtocol.h"
#import "DJDebugImageApiDetailPage.h"

@interface DJDebugImageAlarmInfoPage ()
...
extern NSString *const kDJDebugAPISwitchState;
NSString *const kDJDebugImageSwitchState = @"kDJDebugImageSwitchState";
NSString *const kDJDebugImageSize = @"kDJDebugImageSize";

@implementation DJDebugImageAlarmInfoPage
...
- (void)viewWillAppear:(BOOL)animated {
    BOOL switchState = [[[NSUserDefaults standardUserDefaults] objectForKey:kDJDebugImageSwitchState] boolValue];
    [self.switchView setOn:switchState animated:YES];
    [self switchValueChange:self.switchView];
    
    NSString *imageSizeStr = [[NSUserDefaults standardUserDefaults] objectForKey:kDJDebugImageSize];
    if (imageSizeStr) {
        self.infoLabel.text = [NSString stringWithFormat:@"%@kb",imageSizeStr];
    } else {
        self.infoLabel.text = @"10kb";
        [[NSUserDefaults standardUserDefaults] setValue:@"10" forKey:kDJDebugImageSize];
    }
}
...
- (void)switchValueChange:(UISwitch *)switchView {
    [[NSUserDefaults standardUserDefaults] setValue:@(switchView.on) forKey:kDJDebugImageSwitchState];
    if (switchView.on) {
        [[NSUserDefaults standardUserDefaults] setValue:@(YES) forKey:kDJDebugAPISwitchState];
...
- (void)syncUserfault {
    if (self.imageSizeField.text.length > 0) {
        [[NSUserDefaults standardUserDefaults] setValue:self.imageSizeField.text forKey:kDJDebugImageSize];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.errorApiAllKeys.count > 0) {
        ...
        DJDebugImageApiDetailPage *detailPage = [[DJDebugImageApiDetailPage alloc] init];
