...
@implementation NSObject (DJDebug)
...
+ (SEL)DJDebug_creatGetterWithPropertyName: (NSString *) propertyName{
    //1.返回get方法: oc中的get方法就是属性的本身
    return NSSelectorFromString(propertyName);
}

/** 获取Invocation返回值 */
+ (id)DJDebug_returnPropertyValueWithSig:(NSMethodSignature *)signature
...
+ (NSSet *)DJDebug_propertyKeysWithModel:(id)model {
    NSMutableSet *keys = [NSMutableSet set];
    [self DJDebug_enumeratePropertiesUsingBlock:^(objc_property_t property, BOOL *stop) {
...
+ (void)DJDebug_enumeratePropertiesUsingBlock:(void (^)(objc_property_t property, BOOL *stop))block
                                        model:(id)model {
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    [self DJDebug_modelEncodeWithCoder:aCoder];
}
- (id)initWithCoder:(NSCoder *)aDecoder {
    self = [self init];
    return [self DJDebug_modelInitWithCoder:aDecoder];
}
